...
class GlobalDeployer{
    ...
    constructor(global_shift = new THREE.Vector3(0,0,0), inter_child_space =  new THREE.Vector3(2,0,0), inter_layer_space =  new THREE.Vector3(0,0,-2)){
        ...
        mouseControls.register("render", this.onRender.bind(this));
    }
    ...
    importObject(object, position=new THREE.Vector3(0,0,0), scale=new THREE.Vector3(1,1,1), parent=scene, identifier="", type=classes.Object3D.constructor){
        ...
        let object3D = this.to3D.apply(object, [position, scale, parent, identifier, type]);
        if(parent === scene){
            if(!this.variables[identifier]) this.variables[identifier] = [];
            this.variables[identifier].push(object3D);
        }
        return object3D
    }
        ...
        if(type !== classes.Object3D.constructor){
            ...
            try{
                ...
                if(objectType === classes.Object3D.name){
                    object3D = new deployer.classes[objectType](object, parent, identifier, null, objectType, objectType,{},{},[]);
                ...
                if(e instanceof TypeError){
                    ...
                    let C = geval("class " + objectType + " extends deployer.classes.Object3D{\n" +
                                // "   color = deployer.classes.Color.fromString(\"" + new deployer.classes.Color().randomLight() + "\");\n" +
                                "   constructor(object, parent, identifier){\n" +
                                "       super(object, parent, identifier, null, \""+objectType+"\", \"" + objectType +"\", {},{},[]);\n" +
                                "    }\n" +
                                "   draw(){\n" +
                                "       this.color = deployer.classes.Color.fromString(\"" + new deployer.classes.Color().randomLight() + "\");\n" +
                                "       return super.draw();\n" +
                                "   }\n" +
                                "}; "+ objectType);
    ...
    onRender(object3DList){
        this.hideAllRelations();
        if(object3DList.length > 0){
            this.showRelations(object3DList[0]);
        }
    ...
    startMovingObjects(objects){
        this.grid.setMovingObject(objects);
    }
    waitForUserPickObject(conditions, onClickCallback){
        var object_ids = Object.getOwnPropertyNames(this.objects);
        var objects = this.objects;
        var callback_ids = [];


        for(var object_id of object_ids){
            let object = objects[object_id];
            const handler = function(event, mouse, raycaster){
                if(conditions(object, event, mouse, raycaster)){
                    // onPick();
                    // console.log("PICKED: ", object, " handler: ", this.callback, this.mesh, this.compete);
                    for (var callback_id of callback_ids){
                        mouseControls.unregister("mousedown",callback_id)
                    }
